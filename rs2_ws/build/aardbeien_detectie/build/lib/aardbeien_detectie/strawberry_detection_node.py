...
class FrameStreamer:
    ...
    def start(self):
        cmd = [
            ...
            "-preset",      "ultrafast",
            "-tune",        "zerolatency",
            "-x264-params", "sync-lookahead=0:rc-lookahead=0:sliced-threads=1:bframes=0",
            "-g",           "1",        # elke frame een keyframe
            "-bf",          "0",        # geen B-frames
            "-refs",        "1",        # minimale referentieframes
            "-bufsize",     "0",        # geen encoder buffer
            "-maxrate",     "2M",       # maximale bitrate begrenzen
            "-muxdelay",    "0",        # geen mux delay
            "-muxpreload",  "0",        # geen mux preload
            "-f",           "mpegts",
            f"udp://{self.ip}:{self.port}?pkt_size=1316&buffer_size=0",
        ]
        self._proc = subprocess.Popen(
