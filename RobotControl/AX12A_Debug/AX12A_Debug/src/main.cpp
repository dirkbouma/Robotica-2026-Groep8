...
int readPacket(uint8_t* out_buffer, uint8_t max_len, uint8_t* out_error, uint32_t timeout_ms = 100) {
  ...
  while (millis() - start < timeout_ms) {
    if (DxlSerial.available()) {
      ...
      switch (state) {
        ...
        case 1: if (b == 0xFF) state = 2; else state = 0; break;
        case 2: 
          id = b; 
          calc_checksum += b; 
          state = 3; 
        ...
        case 4: 
          ...
          calc_checksum += b; 
          if (length == 2) state = 6; // No parameters
          else state = 5;
          break;
        case 5: 
          if (param_idx < max_len) out_buffer[param_idx] = b;
          param_idx++;
          calc_checksum += b;
          if (param_idx >= length - 2) state = 6;
          break;
        case 6:
          checksum = b;
...
void printErrorStatus(uint8_t error) {
  if (error == 0) return;
  Serial.print("  [ERRORS]: ");
  if (error & 0x01) Serial.print("InputVoltage ");
  if (error & 0x02) Serial.print("AngleLimit ");
  if (error & 0x04) Serial.print("Overheating ");
  if (error & 0x08) Serial.print("Range ");
  if (error & 0x10) Serial.print("Checksum ");
  if (error & 0x20) Serial.print("Overload ");
  if (error & 0x40) Serial.print("Instruction ");
  Serial.println();
}
...
void cmd_read(uint8_t id, uint8_t addr, uint8_t len) {
  ...
  if (res >= 0) {
    ...
    for (int i = 0; i < res; i++) {
      Serial.printf("  [%02d] 0x%02X (%d)\n", addr + i, rx_buffer[i], rx_buffer[i]);
    }
    printErrorStatus(err);
  } else {
...
void cmd_led(uint8_t id, uint8_t state) {
  cmd_write(id, 25, state ? 1 : 0);
}

void cmd_reset(uint8_t id) {
  Serial.printf("Factory resetting ID %d... (This will reset ID to 1 and baud to 1000000)\n", id);
